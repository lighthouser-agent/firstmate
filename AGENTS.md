...
Tell that worker the invalidation explicitly; its brief owns the supersession sequence (supported abort, custody settled from structured status, rebuild from the correct pre-invalidation base on the pipeline-pushed head, one validation against the final head).
